...
export default function News({ posts, authors }) {
      ...
      if (post) {
        cols.push(
          <div key={`blog-col-${j}`} className="col col-12 col-sm-4">
            <BlogCard key={`post-${post.id}`} post={post} author={authors[post.author]} lang="pt" />
          </div>
        );
      } else {
  ...
  if (!postComponents.length) {
    postComponents.push(<p>Não há novas atualizações disponíveis!</p>);
  }
...
export async function getStaticProps() {
  ...
  posts.forEach((post) => {
    ...
    if (!authors[authorId]) {
      ...
      promises.push(axios.get(`http://wordpress-pt.cpplus.pw/index.php/wp-json/wp/v2/users/${authorId}`));
    }
  });
