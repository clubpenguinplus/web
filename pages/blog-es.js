...
export default function News({ posts, authors }) {
      ...
      if (post) {
        cols.push(
          <div key={`blog-col-${j}`} className="col col-12 col-sm-4">
            <BlogCard key={`post-${post.id}`} post={post} author={authors[post.author]} lang="es" />
          </div>
        );
      } else {
  ...
  if (!postComponents.length) {
    postComponents.push(<p>¡No hay nuevas actualizaciones disponibles!</p>);
  }
...
export async function getStaticProps() {
  ...
  for (let i = 0; i < MAX_PAGES; i++) {
    try {
      const { data } = await axios.get(`$http://wordpress-es.cpplus.pw/index.php/wp-json/wp/v2/posts?_embed`);
  ...
  posts.forEach((post) => {
    ...
    if (!authors[authorId]) {
      ...
      promises.push(axios.get(`$http://wordpress-es.cpplus.pw/index.php/wp-json/wp/v2/users/${authorId}`));
    }
  });
